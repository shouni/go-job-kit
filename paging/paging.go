...
//
// go-utils の jobid.SortKey がそのまま渡せます（このパッケージは stdlib だけに
// 依存するため、既定として組み込むことはしていません）。
...
//
// sortKey は並べ替えに使うキーの取り出し方です。**呼び出し側が必ず選んでください。**
// オプションではなく引数なのは、既定のまま呼べる形にすると、間違った既定のまま
// 呼べてしまうからです。ここで誤ると一覧は正常に返り、順序だけが静かに崩れます。
//
